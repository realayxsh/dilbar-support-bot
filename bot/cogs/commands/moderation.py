...
class Moderation(commands.Cog):
    ...
    async def unlockall(self,
        ...
        try:
            ...
            embed = discord.Embed(
                color=discord.Color.default(), description=f"**Failed to unlock, {server}.**")
...
Reason: `{entry.reason}`\n\n''')
    ...
    async def _lock(self,
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(title="Dilbar Support | Lockdown",
    ...
    async def _unlock(self,
        ...
        try:
            await channel.set_permissions(
                ...
                reason=reason)
            await ctx.send(embed=discord.Embed(
                title="Dilbar Support | Unlockdown",
                description=
            ...
            await ctx.send(
                embed=discord.Embed(title="Dilbar Support | Unlockdown",
...
#
    ...
    async def emojisearch(self, ctx: commands.Context, name: Lower = None):
        ...
        await ctx.reply(embed=discord.Embed(
            description=f"Found `{len(emojis)}` emojis .", color=discord.Color.default()))
    ...
    async def stickersearch(self, ctx: commands.Context, name: Lower = None):
        ...
        await ctx.reply(embed=discord.Embed(
            description=f"Found `{len(embeds)}` stickers .", color=discord.Color.default()))
        if len(embeds) == 1:
...
{"**Related Emoji:** "+":"+sticker.emoji+":" if isinstance(sticker, discord.GuildSticker) else "**Tags:** "+', '.join(sticker.tags)}
                """,
                color=discord.Color.default()).set_thumbnail(url=sticker.url)
