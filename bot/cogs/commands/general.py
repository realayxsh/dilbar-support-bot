...
Aliases""")
    ...
    @commands.hybrid_command(name="token", usage="Token <member>")
    @blacklist_check()
    @ignore_check()
    ...
    @commands.hybrid_command(name="users", help="check users of Dilbar Support .")
    @blacklist_check()
    @ignore_check()
    async def users(self, ctx: commands.Context):
        embed = discord.Embed(
            title=f"**Users:**",
            description=
            f"**{len(set(self.bot.get_all_members()))} Users Of Dilbar Support**",
            color=discord.Color.default())
        await ctx.send(embed=embed)
