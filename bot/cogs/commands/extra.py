...
class Utility(commands.Cog):
    ...
    async def stats(self, ctx):
        """Shows some usefull information about Dilbar Support"""
        ...
        devansh = await self.bot.fetch_user(282494845753491456)
        vinit = await self.bot.fetch_user(282494845753491456)
      

        embed.add_field(
            name='<:developer:1103965691859697725> • **Developers**',
            value=
            f"[{devansh}](https://discord.com/users/282494845753491456)\n"
        )

        embed.set_author(name=f"{self.bot.user.name} Stats",
                         icon_url=self.bot.user.display_avatar.url)
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
        embed.set_footer(text='Thanks For Using Dilbar Support service',
                         icon_url=self.bot.user.display_avatar.url)
    ...
    async def invite(self, ctx: commands.Context):
        embed = discord.Embed(
            description=
            "> • [Click Here To Invite Dilbar Support   To Your Server](https://discord.com/api/oauth2/authorize?client_id=1103575978770436116&permissions=8&scope=bot)\n> • [Click Here To Join My Support Server](https://discord.gg/HyWQdHjNPz)",
            color=discord.Color.default())
    ...
    async def botinfo(self, ctx: commands.Context):
        ...
        embed = discord.Embed(color=discord.Color.default(),
                              title="Dilbar Support Information",
                              description=f"""
...
**Discord.py Version:** {discord.__version__}
    ...
    async def serverinfo(self, ctx: commands.Context):
        ...
        embed = discord.Embed(color=discord.Color.default()).set_author(
...
**Color:** {member.color if member.color else '000000'}
    ...
    async def vcinfo(self, ctx: Context, vc: discord.VoiceChannel):
        e = discord.Embed(title='VC Information', color=discord.Color.default())
    ...
    async def note(self, ctx, *, message):
        ...
        if uid not in data["users"]:
            data["users"][uid] = []
        if len(data["users"][uid]) >= 3:
            await ctx.send("**You cannot add more than 3 notes**")
            return
        data["users"][uid].append(message)
        _save_notes(data)
    ...
    async def notes(self, ctx):
        ...
        if not user_notes:
            ...
            embed = discord.Embed(title="Notes",
                                  ...
                                  color=discord.Color.default())
            for i, msg in enumerate(user_notes, start=1):
                embed.add_field(name=f"Note {i}", value=msg, inline=False)
            await ctx.send(embed=embed)
    ...
    async def trashnotes(self, ctx):
        data = _load_notes()
        uid = str(ctx.author.id)
        if uid in data["users"] and data["users"][uid]:
            data["users"][uid] = []
            _save_notes(data)
