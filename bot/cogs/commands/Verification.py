...
class Verification(commands.Cog):
  ...
  async def verify_enable(self, ctx, channel: discord.TextChannel, role: discord.Role):
    ...
    if str(ctx.guild.id) in data:
      ...
      view = View()
      view.add_item(btn)
      embed = discord.Embed(title='Verification', description=f"To Access In *{ctx.guild.name}* Press the <:icon_verified:1104655495618379786> Button Below", color=discord.Color.default())
  ...
  async def verify_config(self, ctx):
    with open("verification.json", "r") as f:
      data = json.load(f)
    embed = discord.Embed(title="Verification Configuration", color=discord.Color.default())
